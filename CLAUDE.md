...
- **`seen` records what was on the ballot at submit, unioned across submissions.** `visibleTo` in `response.go` is the definition the *seen set* uses; `Ballot`/`BallotFor` in `survey.go` derive display order independently and are what a respondent actually sees. `TestBallotAndVisibleToAgreeOnWhatIsOnTheBallot` enforces that the two admit the same options — change one and the other must follow. Prefer `visibleTo` in new code rather than re-deriving approved-plus-own-pending. For every approved option `Votes <= Shown <= respondents`, on every path that writes a response: submission, write-in, backfill and **restore-from-backup** (`portable.go` carries `seen`; a new child table of `responses` has to be taught to every writer of one).
